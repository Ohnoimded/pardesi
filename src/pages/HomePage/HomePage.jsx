...
const HomePage = () => {
    ...
    const getNews = async (pageNum) => {
        try {
            if (pageNum <= 12) {
                ...
                setIsLoading(false);
                if (pageNum >= 1 && !isInitPageReady) {
                    setIsInitPageReady(true);
                }
                if (pageNum >= 1 && !isInitPageReady) {
                    setIsInitPageReady(true);
                }
                if (pageNum >= 3 && !isRestPageReady) {
                    setIsRestPageReady(true);
                }
    ...
    return (<>
        ...
        {isRestPageReady && isInitPageReady ? <>
            <Hr></Hr>
            ...
            {loadedNews.length > 10 && (
                <ScrollToTop smooth component={<ChevronUp />} style={{ boxShadow: 'none', display: 'flex', backgroundColor: 'var(--fr-blue)', color: 'var(--fr-white)', justifyContent: 'center', alignItems: 'center', borderRadius: '15px', zIndex: '10000000', marginBottom: '15px' }} />
            )}
        </>
            : <></>
        }
